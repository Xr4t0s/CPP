...
static void verifyValueFormat(const std::string& value, const std::string& line, uint nLine) {
	...
	if (value.empty())
		buildErr(BAD_VALUE, line, nLine, "Invalid value format");
}

static void checkDateWindow(
	...
	for (int i = 0; year[i]; i++) {
		if (!isdigit(year[i]))
		buildErr(BAD_DATE, line, nLine, "Not a valid year");
	}
...
static void	verifyDateFormat(std::string& date, const std::string& line, uint nLine) {
	std::size_t pos = date.find('-');
	if (pos == std::string::npos || pos != 4)
		buildErr(BAD_DATE, line, nLine, "Not a valid data");
	...
	checkDateWindow(year, month, day, line, nLine);
}
...
void	processLine(std::string line, uint nLine) {
	...
	verifyDateFormat(date, line, nLine);
	verifyValueFormat(value, line, nLine);
	...
	std::cout << date << " => "	<< value << " = " << it->second * converted_value << std::endl;
}
